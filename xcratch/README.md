...
この値は一覧のカードの説明の後ろに `(v0.3.0)` の形で出る。HID 版と Remap3 版の両方に同じ番号が出る。
**デスクトップ版のカードも同じ値を読んでいる**（`scratch-gui/src/lib/libraries/extensions/index.jsx`）。
番号はここ 1 か所にしか書かないので、Xcratch 版とデスクトップ版で番号がずれることはない。
リポジトリに新しいタグを打つときは、ここも一緒に上げてビルドし直す（デスクトップ版もビルドし直す）。
